...
Ziffern $1, 2, 3, 4$ dort gesetzt sein.

\textit{Klauselzahl:} 4~Zeilen $\times$ 4~Spalten $= 16$~Felder,
eine Klausel pro Feld $\Rightarrow$ \textbf{16~Klauseln}.
...
\]
In Worten: F\"ur jedes Feld $(i,j)$ und jedes Ziffern-Paar $n < m$
d\"urfen nicht beide gleichzeitig in diesem Feld stehen.

\textit{Klauselzahl:} 16~Felder $\times$ $\binom{4}{2} = 6$~Ziffern-Paare
pro Feld $\Rightarrow$ \textbf{96~Klauseln}.
...
\textit{Klauselzahl:} 4~Zeilen $\times$ 4~Ziffern $\times$
$\binom{4}{2} = 6$~Spalten-Paare pro Zeile
$\Rightarrow$ \textbf{$4 \cdot 4 \cdot 6 = 96$~Klauseln}.
...
\textit{Klauselzahl:} 4~Spalten $\times$ 4~Ziffern $\times$
$\binom{4}{2} = 6$~Zeilen-Paare pro Spalte
$\Rightarrow$ \textbf{$4 \cdot 4 \cdot 6 = 96$~Klauseln}.
...
\textit{Klauselzahl:} 4~Regionen $\times$ 4~Ziffern $\times$
$\binom{4}{2} = 6$~Feld-Paare pro Region
$\Rightarrow$ \textbf{$4 \cdot 4 \cdot 6 = 96$~Klauseln}.
...
\medskip
\textbf{Notation des Schaltkreises.}
\begin{itemize}[nosep]
\item Jede Komponente (Gate) hat \textbf{Inputs links} und \textbf{einen Output rechts}.
\item $[>k](x_1, \ldots, x_n)$: Output $= 1$, wenn die Summe
      $x_1 + \cdots + x_n$ \emph{strikt gr\"o\ss er} als $k$ ist,
      sonst $0$.
\item $[=k](x_1, \ldots, x_n)$: Output $= 1$, wenn die Summe
      \emph{genau gleich} $k$ ist, sonst $0$.
\item Gabelungen von Leitungen sind mit einem \textbf{Punkt}
      markiert; Kreuzungen \emph{ohne} Punkt bedeuten, dass die
      Leitungen \textbf{nicht verbunden} sind.
\item Alle Signale sind bin\"ar: $A, B, C, D, E, F \in \{0, 1\}$.
\end{itemize}

\medskip
\textbf{Variablen-Legende.}
\begin{itemize}[nosep]
\item $A, B, C$: die drei \textbf{Eing\"ange} des ganzen Schaltkreises.
\item $D, E, F$: die drei \textbf{Ausg\"ange} des ganzen Schaltkreises.
\item $o_1, o_2, o_3$: \textbf{Hilfsvariablen} f\"ur die Outputs der
      drei linken Gates, damit man die Formel nicht zu lang wird.
\end{itemize}
...
F\"ur bin\"are Inputs ($0$ oder $1$) wird jede Summen-Bedingung in
einen logischen Ausdruck \"ubersetzt:

\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{l|l|l}
\toprule
\textbf{Bedingung} & \textbf{\"Aquivalenter Ausdruck} & \textbf{Begr\"undung} \\
\midrule
$A + B > 1$
  & $A \wedge B$
  & Summe $> 1$ mit 2~Inputs $\Leftrightarrow$ beide sind $1$ \\
$A + C = 1$
  & $(A \wedge \neg C) \vee (\neg A \wedge C)$
  & Summe $= 1$ $\Leftrightarrow$ genau einer ist $1$ (XOR) \\
$B + C > 0$
  & $B \vee C$
  & Summe $> 0$ $\Leftrightarrow$ mindestens einer ist $1$ \\
$o_1 = 0$
  & $\neg o_1$
  & $o_1 = 0$ $\Leftrightarrow$ $o_1$ ist falsch \\
$o_1 + o_2 + o_3 > 2$
  & $o_1 \wedge o_2 \wedge o_3$
  & Summe $> 2$ mit 3~Inputs $\Leftrightarrow$ alle drei sind $1$ \\
$o_2 + o_3 > 1$
  & $o_2 \wedge o_3$
  & Summe $> 1$ mit 2~Inputs $\Leftrightarrow$ beide sind $1$ \\
\bottomrule
...
F\"ur jeden Output wird eine \"Aquivalenz zwischen der
Output-Variable und dem Ausdruck aus Schritt~2 geschrieben. Alle
sechs \"Aquivalenzen werden mit $\wedge$ verkn\"upft:

\begin{align*}
\Phi \;\equiv\; & (o_1 \leftrightarrow (A \wedge B))
  && \text{(Gate $[>1]$ oben links)} \\
& \wedge\; (o_2 \leftrightarrow ((A \wedge \neg C) \vee (\neg A \wedge C)))
  && \text{(Gate $[=1]$ mitte links)} \\
& \wedge\; (o_3 \leftrightarrow (B \vee C))
  && \text{(Gate $[>0]$ unten links)} \\
& \wedge\; (D \leftrightarrow \neg o_1)
  && \text{(Gate $[=0]$, Output $D$)} \\
& \wedge\; (E \leftrightarrow (o_1 \wedge o_2 \wedge o_3))
  && \text{(Gate $[>2]$, Output $E$)} \\
& \wedge\; (F \leftrightarrow (o_2 \wedge o_3))
  && \text{(Gate $[>1]$, Output $F$)}
\end{align*}

In Worten: ``$o_1$ ist genau dann $1$, wenn $A$ und $B$ beide $1$
sind'' usw.\ -- jede Zeile codiert die Funktionsweise eines Gates.
Ein Modell von $\Phi$ (projiziert auf $A, B, C, D, E, F$) entspricht
genau einer zul\"assigen Schaltkreis-Konfiguration.
...
$\Phi$ beantworten. Die Grundidee:

\begin{itemize}[nosep]
\item ``Kann Bedingung $X$ im Schaltkreis vorkommen?'' $\Rightarrow$
      Pr\"ufe $\Phi \wedge X$ auf Erf\"ullbarkeit.
      \emph{Erf\"ullbar} = ja, es gibt eine Belegung.
\item ``Gilt Bedingung $Y$ in jeder Konfiguration?'' $\Rightarrow$
      Pr\"ufe $\Phi \wedge \neg Y$ auf Erf\"ullbarkeit.
      \emph{Unerf\"ullbar} = ja, $Y$ gilt immer.
\end{itemize}
...
\toprule
 & \textbf{Frage} & \textbf{Dem Tester geben} & \textbf{Antwort, wenn \ldots} \\
\midrule
(a) & Kann $F = 1$ sein?
    & $\Phi \wedge F$
    & erf\"ullbar $\Rightarrow$ ja \\
(b) & Kann $E = A$ sein?
    & $\Phi \wedge (E \leftrightarrow A)$
    & erf\"ullbar $\Rightarrow$ ja \\
(c) & Gilt $F = 1 \Leftrightarrow D = 0$?
    & $\Phi \wedge \neg(F \leftrightarrow \neg D)$
    & unerf\"ullbar $\Rightarrow$ ja \\
\bottomrule
\end{tabular}
\end{center}
...
W\"ahle $A = 1, B = 1, C = 0$. Einsetzen in die Gate-Formeln:
\begin{align*}
o_2 &= (A \wedge \neg C) \vee (\neg A \wedge C)
     = (1 \wedge 1) \vee (0 \wedge 0) = 1 \\
o_3 &= B \vee C = 1 \vee 0 = 1 \\
F   &= o_2 \wedge o_3 = 1 \wedge 1 = 1 \;\checkmark
\end{align*}
Damit ist $\Phi \wedge F$ erf\"ullbar.

\smallskip
\textbf{(b) Ja, $E = A$ ist m\"oglich.}\\
Einfachster Fall $A = 0$: w\"ahle $A = B = C = 0$.
Dann $o_1 = 0$, also $E = o_1 \wedge o_2 \wedge o_3 = 0 = A$. \checkmark\\
Auch f\"ur $A = 1$ gibt es eine Belegung:
$A = 1, B = 1, C = 0 \Rightarrow o_1 = 1, o_2 = 1, o_3 = 1$,
also $E = 1 = A$. \checkmark\\
Damit ist $\Phi \wedge (E \leftrightarrow A)$ erf\"ullbar.

\smallskip
\textbf{(c) Nein, $F = 1 \Leftrightarrow D = 0$ gilt NICHT.}\\
Gesucht ist ein Gegenbeispiel (eine Belegung, die $\Phi$ erf\"ullt,
aber $F \leftrightarrow \neg D$ verletzt).
