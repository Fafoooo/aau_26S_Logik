...
\smallskip
\textit{Notation:} In den folgenden Formeln laufen die Indizes jeweils
von $1$ bis $4$:
\begin{itemize}[nosep]
\item $i, k$: Zeilen-Indizes
\item $j, k$: Spalten-Indizes \;(Buchstabe $k$ je nach Kontext Zeile oder Spalte)
\item $n, m$: Ziffern aus $\{1,2,3,4\}$
\item $r$: Regions-Index \;(w\"ahlt eine der 4 Regionen $R_1, \ldots, R_4$)
\item $(i,j), (k,l)$: Koordinaten zweier Felder im selben Bereich
\end{itemize}
...
verschiedene Felder in dieser Zeile (Spalten $j \ne k$) d\"urfen
nicht beide Felder die Ziffer $n$ enthalten.
...
verschiedene Felder in dieser Spalte (Zeilen $i \ne k$) d\"urfen
nicht beide Felder die Ziffer $n$ enthalten.
$\Rightarrow$ $4 \cdot 4 \cdot 6 = 96$ Klauseln.
...
\bigwedge_{\substack{(i,j),(k,l) \in R_r \\ (i,j) \neq (k,l)}}
\neg\bigl(x_{i,j,n} \wedge x_{k,l,n}\bigr).
\]
In Worten: F\"ur jede Region $R_r$, jede Ziffer $n$ und je zwei
verschiedene Felder $(i,j), (k,l)$ in dieser Region d\"urfen nicht
beide Felder die Ziffer $n$ enthalten.
$\Rightarrow$ $4 \cdot 4 \cdot 6 = 96$ Klauseln.
